...
**描述**
 ...
 给 a、b 两个文件，各存放 50 亿个 URL，每个 URL 各占 64B，内存限制是 4G。请找出 a、b 两个文件共同的 URL。
...
对于本题，8 位电话号码可以表示的号码个数为 99999999个，即 1 亿个。我们每个号码用一个 bit 来表示，则总共需要 1 亿个 bit，内存占用约 100M。
...
对于这道题，顺序读取这 5 亿个数字，对于读取到的数字 num，如果它对应的二进制中最高位为1，则把这个数字写到 f1 中，如果数字的最高位为0，则将这个数字写入 file_0文件中。通过这一步，可以把这 5 亿个数划分为两部分，而且 f0 中的数都大于 f1 中的数(int型，第32位，符号位，0是正，1是负)。
...
一个10G的文件，每行都是一个整型数字 提供5G内存，请对文件里的数据进行排序，并输出到一个新的文件
...
归并排序：
 ...
 取10份排序好的数据的首位（分别为每份排序好的数据的最小值）进入最小堆。则最小的数位于堆顶，移除堆顶元素并写入缓冲区，然后从移除元素的元素所属数组中的下一位进入最小堆，在次移除堆顶进入缓冲区...直到缓冲区满，缓冲区回写磁盘，清空缓冲区，再次将数据置入最小堆...
...
整型数字范围为-2的31次方到2的31次方  -  1

内存开辟一个小根堆，小根堆里的每个元素为  数字）  那么每个元素占用4字节   假设1G内存能装2的

28次方个这样的元素。那么我们可以把-2的31次方到2的31次方  -  1 等分为 2的4次方个，那么第一个区间就是-2的31次方到-2的31次方 + 2的28次方，读取文件，如果数字处于这个区间内，就进到小根堆，文件读取完毕，那么此时小根堆中就保存了文件中所有处于-2的31次方到-2的31次方 + 2的28次方区间的数字，并且进行了排序，以此类推。。。
...
某搜索公司一天的用户搜索词汇是海量的（百亿数据量），请设计出求出每天热门top100词汇的可行办法



结局思路

将海量文件根据hash取模分成10000（实际分成多少个看题目要去）个小文件，每个小文件求出top100（hashmap k为词汇 v为词频）

在内存创建大根堆，每个文件的第一个词频入大根堆（大跟堆中10000个词汇），然后弹出堆顶元素，取堆顶元素所属文件的下一个词汇入堆，然后弹出堆顶元素，继续取堆顶元素所属文件的下一个词汇入堆，依次类推。。。。
...
32位无符号整数的范围是0~4294967259，现在有40亿个无符号整数，可以使用最多1GB内存，找出所有出现了两次的数 

方法一：hash取模分解成小文件 每个小文件统计

方法二：00 代表没出现 01代表1次 10代表2次 11代表大于两次 1G装得下
...
10kb内存找40亿个无符号整数的中位数

10kb可以存2500个整型（2048）把0-4294967259等分为2048份 声明一个2048长度的数组arr arr[0]代表0- 2097152这个范围内无符号整数出现的次数，依次类推。遍历40亿个无符号整数，得到每个范围的词频。

从数字的第一个元素开始累加，如果加到第n个元素，大于20亿了，代表中位数就在这个范围，在把这个范围等分为2048份

15

32位无符号整数的范围是0~4294967259，现在有一个包含40亿个数的文件，给你3KB内存，找到一个未出现的数字

3KB的内存创建数组arr，数组的长度为512，把0~4294967259分成512份，则zrr[0]代表0-8388608这个范围的数字出现的次数，依次类推，由于只有40亿个数，那么肯定有某个范围出现的次数没有8388608次。把这个范围在分成512份，继续。。。
